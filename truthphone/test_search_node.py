...
async def test():
    try:
        print("Testing BrowserChatGroq with max_tokens=4096...")
        llm = BrowserChatGroq(model="openai/gpt-oss-20b", max_tokens=4096, temperature=0.1)
        browser = Browser(config=BrowserConfig(headless=False))
        agent = BrowserAgent(
            task="Go to https://www.google.com and search for samsung 5g phone",
            llm=llm,
            browser=browser
        )
        res = await agent.run()
